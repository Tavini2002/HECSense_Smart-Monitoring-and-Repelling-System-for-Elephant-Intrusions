...
def play_alarm_once(sound_path: str, volume: float = 1.0) -> None:
    """
    Start the alarm sound. Plays once until it finishes (non-blocking).
    Only starts if no alarm is currently playing.
    """
    ...
    is_currently_playing = _audio_is_ready and pygame.mixer.music.get_busy()
    
    # Only start a new alarm if one isn't already playing
    if not is_currently_playing:
        try:
            pygame.mixer.music.load(sound_path)
            pygame.mixer.music.set_volume(max(0.0, min(1.0, float(volume))))
            pygame.mixer.music.play(loops=0)  # Play once (loops=0 means play once)
...
def process_video_stream(video_file, conf_threshold, alarm_enabled, alarm_label, alarm_volume):
    ...
    try:
        while not _stop_event.is_set():
            ...
            # Alarm control logic
            # Only start alarm if: alarm is enabled, elephant detected, path exists, and no alarm is currently playing
            if alarm_enabled and elephant_detected and alarm_path:
                # Only start a new alarm if one isn't already playing
                # This allows the alarm to finish playing even if detection fails in some frames
                if not is_alarm_playing():
                    play_alarm_once(alarm_path, volume=alarm_volume)
            # Note: We don't stop the alarm when no elephant is detected
            # The alarm will play until it finishes naturally
